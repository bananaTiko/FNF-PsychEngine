...
class UpdateState extends MusicBeatState
{
	...
	override function create()
	{
		...
		text = new FlxText(0, 0, 0, "Please wait, Untitled Psych Fork is updating...", 18);
	...
	inline function getUpdateLink()
	{
		...
		online_url = "https://github.com/bananaTiko/FNF-PsychEngine/releases/download/" + TitleState.updateVersion + '/Untitled Psych Fork-${getPlatform()}.$fileEnd';
	...
	{
		var path:String = './update/temp/'; // Untitled Psych Fork ' + TitleState.onlineVer + ".zip";
		...
		text.screenCenter(X);
		File.saveBytes(path + "Untitled Psych Fork v" + TitleState.updateVersion + ".zip", fileBytes);
		text.text = "Unpacking update file...";
		text.screenCenter(X);
		// Uncompress.run(File.getBytes(path + "Untitled Psych Fork v" + TitleState.updateVersion + ".zip"))
		JSEZip.unzip(path + "Untitled Psych Fork v" + TitleState.updateVersion + ".zip", "./update/raw/");
